...
def _ensure_dt_index(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure df is indexed by tz-aware UTC DatetimeIndex.
    Accepts either:
      - df.index already datetime-like
      - a 'timestamp' column
    """
...
def _asof_value(s: pd.Series, ts: pd.Timestamp) -> float:
    """
    As-of / last-known-value at ts: take last s[t'] where t' <= ts.
    Returns np.nan if no value exists.
    """
...
def _infer_bars_per_day(idx: pd.DatetimeIndex) -> int:
    """
    Offline: bars_per_day inferred from timestamp spacing; effectively 288 for 5m.
    """
    if len(idx) < 10:
        return 288
...
def _resample_ohlcv_like_offline(df5: pd.DataFrame, rule: str) -> pd.DataFrame:
    """
    Use the repo's indicators.resample_ohlcv() if available (preferred, parity-safe).
    Fallback: pandas resample default label/closed.
    """
    try:
        from live.indicators import resample_ohlcv  # type: ignore
        return resample_ohlcv(df5, rule)
    except Exception:
        # Fallback (only used if indicators.resample_ohlcv is missing)
...
def _atr_like_offline(df: pd.DataFrame, length: int) -> pd.Series:
    """
    Use the repo's indicators.atr() if available (preferred, parity-safe).
    Fallback: Wilder ATR (RMA) with min_periods=length.
    """
    try:
        from live.indicators import atr  # type: ignore
...
def map_to_left_index(target_index: pd.DatetimeIndex, s: pd.Series) -> pd.Series:
    """
    Offline map_to_left_index() semantics: forward-fill reindex.
    """
...
def compute_asset_macd_4h_features(df5: pd.DataFrame, decision_ts: pd.Timestamp) -> Dict[str, float]:
    ...
    Offline definition:
      ...
      EMA_fast = ewm(span=12, adjust=False, min_periods=12)
      EMA_slow = ewm(span=26, adjust=False, min_periods=26)
      macd_line = fast - slow
      macd_signal = ewm(macd_line, span=9, adjust=False, min_periods=9)
      macd_hist = macd_line - macd_signal
      macd_slope = macd_hist.diff(3)
      map_to_left_index(df5.index, series_4h) (ffill)
    """
...
def compute_entry_quality_required_features(
    ...
    """
    Implements offline definitions for:
      prior_1d_ret, rv_3d, consolidation_range_atr, days_since_prev_break,
      don_break_level, don_dist_atr, gap_from_1d_ma
    """
    df5 = _ensure_dt_index(df5)
    df5_upto = df5.loc[:decision_ts]
    if len(df5_upto) == 0:
    ...
    # 3) consolidation_range_atr
    win_bars = getattr(cfg, "PULLBACK_WINDOW_BARS", None)
    if win_bars is None:
        # Offline mentions fallback from hours; default 12 bars
        win_bars = 12
    ...
    # 4) days_since_prev_break (Donch “touch” of prior N-day daily-high ceiling)
...
def compute_cross_asset_daily_context(
    ...
    """
    Offline cross-asset daily context for BTC/ETH:
      df1d = resample_ohlcv(df5, "1D")
      atr_daily = atr(df1d, 20)
      atr_pct_1d = atr_daily / close_1d
      baseline = expanding_median(atr_pct_1d, min_periods=50)
      vol_regime_level = (atr_pct_1d / baseline).shift(1)
      trend_slope = (MA20 - MA50).diff().shift(1)
      map_to_left_index(ts5, series_1d) (ffill)
    """
    df5 = _ensure_dt_index(df5)
    df5_upto = df5.loc[:decision_ts]
    if len(df5_upto) == 0:
