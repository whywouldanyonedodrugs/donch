...
class OHLCVSqliteStore:
    """
    Single-file SQLite store for OHLCV.

    Schema:
      ohlcv(symbol TEXT, tf TEXT, ts INTEGER(ms), open, high, low, close, volume)

    Invariants:
      - ts is candle OPEN timestamp in UTC (matches df index in live_trader).
      - Caller is responsible for ensuring the last incomplete bar has been dropped.
    """
    ...
    def load(
        ...
        *,
        ...
        # Basic validation: monotonic & no duplicates
        df = df.sort_index()
    ...
    def _trim_locked(self, symbol: str, tf: str, keep_last: int) -> None:
        # Find the cutoff ts for the last N bars, then delete older rows.
        cur = self._conn.cursor()
        cur.execute(
            """
            SELECT ts
