...
class LiveTrader:
    ...
    def _load_meta_sizing_curve(self, path=None) -> None:
        """
        Load offline sizing_curve.csv that maps calibrated probability (p_cal) -> risk_multiplier.

        Supported formats:
        - Binned curve: columns include (bin_low, bin_high) or (p_lo, p_hi) plus a multiplier column.
        - Point curve: columns include p/prob (or mean_score) plus a multiplier column.

        Non-fatal: on any issue we log and leave meta sizing disabled.
        """
        from pathlib import Path

        # Reset any previous curve
        ...
        # --- Format A: BINNED curve (lo/hi + multiplier) ---
        lo_col = None
        hi_col = None
        ...
        # --- Format B: POINT curve (p/prob + multiplier) ---
        pcol = None
    ...
    def _resolve_meta_multiplier(self, p_cal: float, risk_on_value: float) -> float:
        """
        Resolve meta sizing multiplier with risk-off probe capping.
        """
        meta_mult = 1.0
    ...
    async def _scan_symbol_for_signal(
        ...
        try:
            ...
            # --- Strict-parity: enforce tz-aware UTC index ---
            if df5.index.tz is None:
                # If your fetcher is already UTC but naive, localize; otherwise fix fetcher upstream.
            ...
            # --- Strict Parity: Inject Robust Breakout Level ---
            don_days = int(self.cfg.get("DON_N_DAYS", 20))
            don_upper_s = donchian_upper_days_no_lookahead(df5["high"], don_days)

            # decision_ts must exist; if not, fallback to last index
            ...
            try:
                if getattr(self, "golden_store", None) is not None:
                    # Decision timestamp = last completed 5m bar timestamp
                    ts = df5.index[-1]
            ...
            # Offline risk-on flags (used by sizing: risk_off -> probe cap)
            try:
            ...
            # Attach donch fields for downstream logging/telemetry
            # (Signal class may not declare them explicitly, so we attach dynamically)
